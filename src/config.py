...
def get_frontend_config() -> dict:
    ...
    # Knowledge (Ezra) is an Action Board tool, not an MC tab — More menu is
    # already tight. Drop a leftover overlay tab if one is still listed.
    tabs = [t for t in tabs if (t.get("id") if isinstance(t, dict) else t) != "knowledge"]
