...
def build_cove_caddy_snippet(*, cove_id: str, domain: str, app_port: int,
                             ...
                             voice_port: int = 0, umami_port: int = 0) -> str:
    ...
    voice., analytics.) win over the *.{domain} wildcard, matching the founder
    Clearfield pattern. voice_port routes voice.{domain} → the Cove's own voice
    container; umami_port routes analytics.{domain} → Umami (script.js + /api/send).
    """
    blocks = [
        f"# ============================================================",
    ...
    if umami_port:
        blocks += [
            "",
            f"# Umami analytics — public collect endpoint for Site Builder snippets.",
            f"# Browsers load /script.js and POST /api/send here (cookieless).",
            f"analytics.{domain} {{",
            f"    reverse_proxy localhost:{umami_port}",
            f"}}",
        ]
    blocks += [
        "",
...
def build_haven_cove_snippet(*, cove_id: str, domain: str, app_port: int,
                             ...
                             umami_on: bool = True, umami_internal_port: int = 3000,
                             acmedns: dict | None = None,
    ...
    umami_target = f"{cove_id}-umami:{umami_internal_port}"
    ...
    if umami_on:
        lines += [
            "",
            f"# Umami analytics — public collect (script.js + /api/send).",
            f"analytics.{domain} {{",
            *_tls_block(),
            f"    reverse_proxy {umami_target}",
            f"}}",
        ]
    lines += [
        "",
