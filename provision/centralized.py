    ...
    data = {
        ...
        "tools": {
            "modules": [
                ...
                "tools.backlog_tools",  # Presence /backlog — own NC board mint
            ],
            "approval_tiers": {
...
""" if matrix_on else "")
  ...
  voice:
    ...
    environment:
      ...
      # place AND publishes multi-GB outputs (caption-full) via shutil.copy2.
      # Must be read-write: :ro forces WebDAV fallback which fails on ~1GB+ puts
      # (EROFS → WebDAV → publish False; file left in /tmp/cove-out). See
...
def build_env(cove: dict, op: dict, providers: list, ltp: dict, mx: dict, deploy: dict,
    ...
    lines = [
        ...
        # Clone path for System → Runbooks path fill ({{CLONE_DIR}}). Same source as
        # deploy.lucid_cove_path / cove_core_path that install.sh passes the provisioner.
        f"COVE_CLONE_DIR={(deploy.get('lucid_cove_path') or deploy.get('cove_core_path') or '').strip()}",
        f"POSTGRES_USER={cid}",
        ...
        # #D35 — Caddy admin token (shared with lucidcove-caddy / bundled caddy).
        # Minted in generate_cove (or inherited) so set-address /load can authenticate
        # the token-gated :2019 proxy. Empty only when no Caddy admin path is used.
        f"LP_CADDY_ADMIN_TOKEN={(os.getenv('LP_CADDY_ADMIN_TOKEN') or '').strip()}",
...
def generate_cove(cfg: dict, out_root: Path) -> dict:
    ...
    # #D35 — mint ONE Caddy admin token for this box and put it in the provisioner
    # env BEFORE build_env + shared-Caddy bootstrap, so:
    #   • the Cove stack .env gets LP_CADDY_ADMIN_TOKEN
    #   • the shared Caddy base Caddyfile renders in token mode (loopback admin +
    #     bearer proxy on :2019)
    #   • out/_shared-caddy/.env carries the same secret for install.sh to copy
    # Without this, set-address POSTs /load with no usable gate path and Caddy's
    # #D32 enforce_origin rejects origin '' → hard onboarding 403.
    if shared_net or bundle_caddy:
        _caddy_tok = (os.getenv("LP_CADDY_ADMIN_TOKEN") or "").strip()
        if not _caddy_tok:
            _boot_env = out_root / "_shared-caddy" / ".env"
            if _boot_env.is_file():
                try:
                    for _line in _boot_env.read_text().splitlines():
                        if _line.startswith("LP_CADDY_ADMIN_TOKEN="):
                            _caddy_tok = _line.split("=", 1)[1].strip()
                            break
                except Exception:
                    _caddy_tok = ""
        if not _caddy_tok:
            _caddy_tok = gen_secret(32)
        os.environ["LP_CADDY_ADMIN_TOKEN"] = _caddy_tok
    ...
    if shared_net:
        ...
        # Box-level admin token — same value the Cove stack .env carries. install.sh
        # copies this to ~/.lucidcove/caddy/.env so compose injects it into
        # lucidcove-caddy. Without it the gate is off and set-address 403s.
        _shared_tok = (os.getenv("LP_CADDY_ADMIN_TOKEN") or "").strip()
        if _shared_tok:
            (scd / ".env").write_text(
                "# Generated by provision — do not commit. Shared with every Cove on this box.\n"
                f"LP_CADDY_ADMIN_TOKEN={_shared_tok}\n"
            )
        base_caddyfile = scd / "Caddyfile"
        # Token in env → render #D35 token-mode base (loopback admin + :2019 proxy).
        # Rewrite when we have a token so a re-provision upgrades a stale #D32 base;
        # conf.d snippets are separate and never clobbered here.
        if _shared_tok or not base_caddyfile.exists():
